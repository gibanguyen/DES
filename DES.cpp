...
// Function to convert bits of char to string
string convertBitsOfCharToString(char p[]) {
	string bits_string;

	for (size_t i = 0; i < sizeof(p); i++) {
		int ascii_code = (short)p[i];
		bitset<8> bits(ascii_code);
		bits_string += bits.to_string();
	}

	return bits_string;
}

// Function to convert a bits string to char
void convertBitsStringToChar(string s, char* p) {
	int pos = 0;
	string temp = "";
	...
	for (int i = 1; i <= s.length(); i++) {
		...
		if (i % 8 == 0 && i != 0) {
			bitset<8> bits(temp);
			p[pos] = static_cast<unsigned char>(bits.to_ulong());
			temp = "";
			pos++;
		}
	}
...
int main() {	
	...
	string bits_key = convertBitsOfCharToString(key);
	...
	for (int i = 0; i < sizeof(pt); i++)
		cout << pt[i] << " ";
	cout << endl;
	cout << "Plain-text (Hexa): " << endl;
	performInHex(pt);

	string bits_pt = convertBitsOfCharToString(pt);

	// A block of ciper text of 64 bits
	char ct[64];

	// Applying the algo
	string bits_ct = DES(bits_pt, bits_key);
	convertBitsStringToChar(bits_ct, ct);

	cout << "Cipher-text (Dec): " << endl;
	for (int i = 0; i < sizeof(ct); i++)
		cout << ct[i] << " ";
	cout << endl;
	cout << "Cipher-text (Hexa): " << endl;
	performInHex(ct);
}